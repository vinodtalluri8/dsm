...
export class AddSystemCodesComponent implements OnInit {
  ...
  constructor(private router: Router, private route: ActivatedRoute, private departmentService: DepartmentsService,
    ...
    private updatesystemcodeService: UpdatesystemcodeService, private confirmationService: ConfirmationService) {
  ...
  validate() {
    ...
    } else if (this.showdepartment && !this.selectedDepartments) {
      return false;
    } else {
      return true;
    }
  ...
  updatesystemcodeval() {
    if (!this.buttonDisabled) {
      this.dataJson = {
        'auditId': this.updateRecordDate['auditId'],
        'value': this.valueinput,
        'guiLabel': this.guilabel,
        'subType': this.showdepartment ? this.selectedDepartments : this.code,
        'description': this.description,
      };
      this.updatesystemcodeService.updateSystemValueData(this.dataJson)
        .subscribe(data => {
  ...
  addSystemCode() {
    if (!this.buttonDisabled) {
      ...
      this.addsystemcodeService.addSystemCodeVal(this.dataJson)
        .subscribe(data => {
          this.savedRecord = data;
          this.saved = true;
