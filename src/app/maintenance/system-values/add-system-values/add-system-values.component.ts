...
export class AddSystemValuesComponent implements OnInit {
  ...
  constructor(private router: Router, private route: ActivatedRoute, private location: Location,
    ...
    private updatesystemvalueService: UpdatesystemvalueService, private confirmationService: ConfirmationService) {
  ...
  validate() {
    if ((this.isColumn1Visible) || this.isColumn2Visible
      || (this.isColumn3Visible && this.column3.trim().length === 0) || (this.isColumn4Visible && this.column4.trim().length === 0)
    || (this.isColumn5Visible && this.column5.trim().length === 0) ||
    (this.isColumn6Visible && this.column6.trim().length === 0)) {
      return false;
  ...
  disable(operation) {
    ...
    if (operation === 'add') {
      this.buttonDisabled = !this.addSystemValueForm.valid;
    } else if (operation === 'update' && this.updateRecordDate) {
      this.buttonDisabled = !(this.addSystemValueForm.dirty || this.description !== this.updateRecordDate['description']);
    }
  ...
  addSystemValues() {
    if (!this.buttonDisabled) {
      ...
      this.addsystemvaluesService.addSystemVal(JSON.stringify(this.dataJson)).subscribe(data => {
  ...
  updateSystemValues() {
    if (!this.buttonDisabled) {
      this.generateListJson();
      this.dataJson = {
        'valueId': this.updateRecordDate['valueId'],
        'code': this.code,
        'description': this.description,
        'list' : this.listJson,
        'column1value': this.column1,
      ...
      this.updatesystemvalueService.updateSystemValue(this.dataJson).subscribe(
        data => {
  ...
  /** This method will perform cancel confirmation**/
  cancelRecord() {
    this.msgs = [];
